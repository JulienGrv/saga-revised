...
function QUEST_START(cid)
	Saga.AddStep(cid, QuestID, 37401);
	Saga.AddStep(cid, QuestID, 37402);
	Saga.AddStep(cid, QuestID, 37403);
...
function QUEST_STEP_2(cid, StepID)
	-- Find Leaked Box of Bullets (8)
	Saga.FindQuestItem(cid, QuestID, StepID, 56, 51700010, 10000, 8, 1);
...
function QUEST_STEP_3(cid, StepID)
	-- Report result to Portman
	Saga.AddWaypoint(cid, QuestID, StepID, 1, 1135);
	
	-- Check for completion
	local ret = Saga.GetNPCIndex(cid);
	if ret == 1135 then
		Saga.GeneralDialog(cid, 3731);
	
		local ItemCountA = Saga.CheckUserInventory(cid, 51700010);
		if ItemCountA > 7 then
			Saga.NpcTakeItem(cid, 51700010, 8);
...
function QUEST_CHECK(cid)
	...
	if CurStepID == 37401 then
		ret = QUEST_STEP_1(cid, StepID);
	elseif CurStepID == 37402 then
		ret = QUEST_STEP_2(cid, StepID);
	elseif CurStepID == 37403 then
