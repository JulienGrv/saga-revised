...
function QUEST_START(cid)
	Saga.AddStep(cid, QuestID, 17501);
	Saga.AddStep(cid, QuestID, 17502);
...
function QUEST_STEP_2(cid, StepID)
	-- Report result to Monika Reynolds
	Saga.AddWaypoint(cid, QuestID, StepID, 1, 1012);
	
	-- Check for completion
	local ret = Saga.GetNPCIndex(cid);
	if ret == 1012 then
		Saga.GeneralDialog(cid, 3321);
		Saga.SubstepComplete(cid, QuestID, StepID, 1);
	end
...
function QUEST_CHECK(cid)
	...
	if CurStepID == 17501 then
		ret = QUEST_STEP_1(cid, StepID);
	elseif CurStepID == 17502 then
